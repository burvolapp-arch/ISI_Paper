...
inherent to the methodology and cannot be resolved by improved parameter
tuning within the current framework.
...
\begin{enumerate}[label=\textbf{L\arabic*}.]
  ...
  \item \textbf{Re-export and entrepôt masking.}
    ...
    5~(critical raw materials), and 6~(logistics) most acutely.  The \ISI
    does not strip out re-export flows because no harmonised re-export
    adjustment exists across all source datasets.
  ...
  \item \textbf{Geographic determinism (Axis~6).}
    ...
    reflect physical proximity, not strategic vulnerability.  The index
    does not adjust for geographic proximity or contiguity.
  ...
  \item \textbf{No domestic production offset.}
    ...
    share domestically will receive the same axis score as a country with
  ...
  \item \textbf{Intra-EU trade inclusion.}
    ...
    EU~partner (e.g., Germany) will receive a high \HHI, even though the
