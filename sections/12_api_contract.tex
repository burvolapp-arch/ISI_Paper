...
\begin{itemize}
  \item \textbf{Protocol.}  HTTPS (TLS~1.2 or later).  Plaintext HTTP
    ...
    endpoint.
  \item \textbf{Content type.}  All responses use
    \texttt{Content-Type:\ application/json;\ charset=utf-8} unless
...
\begin{description}[style=nextline, leftmargin=5em, labelwidth=4.5em]
  ...
  \item[\texttt{format}]  Optional.  One of \texttt{json} (default) or
    \texttt{csv}.  When \texttt{csv}, the response uses
    \texttt{Content-Type:\ text/csv;\ charset=utf-8} and returns the
