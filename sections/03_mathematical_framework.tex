...
into concentration scores on the unit interval.  All notation introduced here
applies throughout the remainder of the document.
...
The threshold values represent a proportional transposition of the
US~Department of Justice / FTC Horizontal Merger Guidelines \HHI bands
\parencite{doj2023merger} to the fractional scale, with an additional tier
boundary at~\num{0.15} introduced to separate mildly concentrated from
unconcentrated states.  They are defined in the version-controlled methodology
registry and enforced by a single classification function in the classification
module.  The thresholds are fixed parameters of \ISI v\,1.0.  Any modification to the
