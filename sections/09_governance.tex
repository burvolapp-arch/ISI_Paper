...
\paragraph{Version-definition immutability.}
Once a version number has been assigned to a published snapshot, the
association between that version number and the corresponding methodology
parameters SHALL NOT be altered.  The methodology registry SHALL be
append-only: existing entries SHALL NOT be mutated or removed.  This
rule applies to all three version components (major, minor, patch).
