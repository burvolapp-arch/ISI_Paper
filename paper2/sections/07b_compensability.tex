...
country using three metrics:

\begin{enumerate}[leftmargin=2em]
  \item \textbf{Max--composite gap.}  Define
    \begin{equation}\label{eq:compensability-gap}
      G_i = \max_j A_{ij} - \text{ISI}_i\,.
    \end{equation}
    A large gap indicates that the country's highest-axis score is
    far above its composite, meaning that the arithmetic-mean
    aggregation substantially compresses a peak vulnerability.
  \item \textbf{Max--min range.}  Define
    \begin{equation}\label{eq:compensability-range}
      R_i = \max_j A_{ij} - \min_j A_{ij}\,.
    \end{equation}
    The range measures the full span of axis-level variation within a
    country's profile.  A large $R_i$ indicates that the country
    combines an extreme vulnerability on at least one axis with
    near-diversification on another.
  \item \textbf{Coefficient of variation (CV).}  The standard deviation
    of the six axis scores divided by their mean:
    \begin{equation}\label{eq:compensability-cv}
      \text{CV}_i = \frac{\sigma_i}{\bar{A}_i}\,.
    \end{equation}
    A high CV indicates that
    the country's axis profile is highly uneven---i.e., the degree of
    internal compensation is large.  The CV is a normalised
    measure that permits comparison across countries with different
    composite levels.
\end{enumerate}

\subsection{Results}

\input{tables/T17_compensability}

\Cref{tab:compensability} reports all three metrics for all 27~countries,
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \textbf{Sweden} (gap = 0.482, CV = 0.770).  Sweden's
    defence axis (0.881) is the primary driver, with financial
    (0.116) and technology (0.120) well below the composite.
\end{enumerate}

\subsection{Interpretation}

The compensability analysis identifies a systematic pattern: countries
...
weighting (Netherlands: 3.07, Bulgaria: 2.03).  This relationship is structurally expected:
